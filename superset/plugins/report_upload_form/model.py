...
class LocationsModel:
    ...
    def __init__(self):
        self.locations = self.__load_locations()

    def __load_locations(self):
        try:
            locations = pd.read_csv(
                "https://raw.githubusercontent.com/sta-zot/SRW/refs/heads/main/data/Locations.csv"
            )
    ...
    def get_region(self, id: int):
        loc = list(
            self.locations[self.locations["id"] == id][["id", "region"]].itertuples(
                index=False, name=None
            )
        )[0]
        return {"id": loc[0], "name": loc[1]}

    def get_regions(self):
        unique_by_region = self.locations.drop_duplicates(subset=["region"])[
            ["id", "region"]
        ]
        return list(unique_by_region.itertuples(index=False, name=None))

    def get_municipality(self, id: int):
        loc = list(
            self.locations[self.locations["id"] == id][
                ["id", "municipality"]
            ].itertuples(index=False, name=None)
        )[0]
        return {"id": loc[0], "name": loc[1]}

    def get_municipalities(self, region: str = "", id: int = None):
        if region:
            filtred_loc = self.locations[self.locations["region"] == region]
            municipalities = filtred_loc.drop_duplicates(subset=["municipality"])[
                ["id", "municipality"]
            ]
        else:
            municipalities = self.locations.drop_duplicates(subset=["municipality"])[
                ["id", "municipality"]
            ]
        return list(municipalities.itertuples(index=False, name=None))

    def get_settlement(self, id: int):
        settelment = self.locations[self.locations["id"] == id][
            ["id", "type", "settlement"]
        ]
        settelment["combined"] = settelment["type"] + ". " + settelment["settlement"]
        loc = settelment[["id", "combined"]].iloc[0].to_list()
        return {"id": loc[0], "name": loc[1]}

    def get_settlements(self, municipality: str = ""):
        if municipality:
            filtred_data = self.locations[
                self.locations["municipality"] == municipality
            ]
            settlements = filtred_data.drop_duplicates(subset=["settlement"])[
                ["id", "type", "settlement"]
            ]
        else:
            settlements = self.locations.drop_duplicates(subset=["settlement"])[
                ["id", "type", "settlement"]
            ]

        return list(settlements.itertuples(index=False, name=None))
